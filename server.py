...
ACTIVE_PROJECT_DIR = ROOT_DIR / "example_project"
SHEETS_DIR = ACTIVE_PROJECT_DIR / "sheets"
ANNOTATIONS_DIR = ACTIVE_PROJECT_DIR / "annotations"
MANIFEST_PATH = ACTIVE_PROJECT_DIR / "manifest.json"
STATIC_FILES = {
    "/": WEB_DIR / "index.html",
    "/app.js": WEB_DIR / "app.js",
    "/annotator_annotations.js": WEB_DIR / "annotator_annotations.js",
    "/annotator_common.js": WEB_DIR / "annotator_common.js",
    "/annotator_gallery.js": WEB_DIR / "annotator_gallery.js",
...
def list_sheet_files() -> list[Path]:
    return sorted(path for path in SHEETS_DIR.glob("*.png") if path.is_file())
...
class AnnotatorHandler(BaseHTTPRequestHandler):
    server_version = "span-annotator/0.1"
    ...
    def do_GET(self) -> None:
        ...
        if parsed.path.startswith("/sheets/"):
            self._serve_sheet_asset(parsed.path)
            return
    ...
    def do_POST(self) -> None:
        ...
        if not (SHEETS_DIR / sheet_name).is_file():
            ...
            sheets.append(
                {
                    ...
                    "annotation_file": str(annotation_path_for_sheet(path.name).relative_to(ROOT_DIR)),
    ...
    def _serve_sheet_asset(self, request_path: str) -> None:
        relative = request_path.removeprefix("/sheets/")
        target = SHEETS_DIR / unquote(relative)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--project", default=str(ROOT_DIR / "example_project"))
...
def main() -> None:
    ...
    args = parse_args()
    ACTIVE_PROJECT_DIR = Path(args.project).resolve()
    SHEETS_DIR = ACTIVE_PROJECT_DIR / "sheets"
    ANNOTATIONS_DIR = ACTIVE_PROJECT_DIR / "annotations"
    MANIFEST_PATH = ACTIVE_PROJECT_DIR / "manifest.json"
    ANNOTATIONS_DIR.mkdir(parents=True, exist_ok=True)
    ...
    print(f"Project: {ACTIVE_PROJECT_DIR}")
