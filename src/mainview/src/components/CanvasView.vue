...
const layerCursorClass = computed(() => {
	...
	if (activePaintTool.value) return activePaintTool.value === 'erase' ? 'cursor-cell' : 'cursor-crosshair';
...
const isPaintableCurrentSheet = computed(() =>
	!!currentSheet.value && /\.(png|jpe?g|webp)$/i.test(currentSheet.value.absolutePath || currentSheet.value.path),
);
...
function renderDisplayCanvas() {
	...
	if (moving && floatingSelectionCanvas.width && floatingSelectionCanvas.height) {
		const scaleX = backingWidth / sampleCanvas.width;
		const scaleY = backingHeight / sampleCanvas.height;
		ctx.drawImage(
...
function drawGridLines(
	...
	if (!canvasGridEnabled.value || !sampleCanvas.width || !sampleCanvas.height) return;
...
function onKeyDown(e: KeyboardEvent) {
	...
	if (e.code === "Escape" && (pixelSelectionDrag.value || pixelSelectionMove.value || paintPixelSelection.value)) {
		clearPixelSelection();
		renderDisplayCanvas();
		return;
	}
...
watch(
	...
	(sheet, previousSheet) => {
		if (previousSheet && pixelSelectionMove.value) {
			finalizeFloatingSelection({ apply: true, clearSelection: true, sheet: previousSheet });
		}
...
watch(activePaintTool, (tool, previousTool) => {
	if (previousTool === "marquee" && tool !== "marquee" && pixelSelectionMove.value) {
...
function stagePixelFromClient(clientX: number, clientY: number): { x: number; y: number } | null {
	const stageEl = stage.value;
...
function selectionStyle(rect: { x: number; y: number; w: number; h: number } | null) {
	if (!rect) return {};
	return {
...
function pointInRect(point: { x: number; y: number }, rect: { x: number; y: number; w: number; h: number } | null) {
	return !!rect
		&& point.x >= rect.x
		&& point.y >= rect.y
...
function paintFloatingSelectionCanvas(imageData: ImageData) {
	...
	const ctx = floatingSelectionCanvas.getContext("2d", { willReadFrequently: true });
	if (!ctx) return;
	ctx.putImageData(imageData, 0, 0);
}
...
function stampPaintPixel(x: number, y: number, erase: boolean) {
	if (!sampleCanvas.width || !sampleCanvas.height) return;
	const half = Math.floor(Math.max(1, Math.round(paintToolSize.value || 1)) / 2);
...
function drawPaintLine(fromX: number, fromY: number, toX: number, toY: number, erase: boolean) {
	let x0 = fromX;
	let y0 = fromY;
	const x1 = toX;
	const y1 = toY;
...
function currentSheetMimeType(): string {
	const path = (currentSheet.value?.absolutePath || currentSheet.value?.path || "").toLowerCase();
...
function commitSampleCanvasEdit(message: string) {
	...
	applyPaintedSheetImage(sheet, dataUrl, sampleCanvas.width, sampleCanvas.height);
	rebuildPaintPalette();
	renderDisplayCanvas();
...
function resizeCurrentCanvas(width: number, height: number): boolean {
	...
	if (nextWidth === sampleCanvas.width && nextHeight === sampleCanvas.height) return false;
	...
	for (const annotation of annotations.value) {
		...
		annotation.aabb.y = Math.max(0, Math.min(annotation.aabb.y, nextHeight - 1));
		annotation.aabb.w = Math.max(1, Math.min(annotation.aabb.w, nextWidth - annotation.aabb.x));
		annotation.aabb.h = Math.max(1, Math.min(annotation.aabb.h, nextHeight - annotation.aabb.y));
	}
...
function copyPixelSelectionToClipboard(): boolean {
	...
	const imageData = floating?.imageData ?? sampleCtx.getImageData(rect.x, rect.y, rect.w, rect.h);
	pixelClipboard = {
		width: rect.w,
		height: rect.h,
		sourceX: rect.x,
...
function deletePixelSelectionPixels(options?: { keepSelection?: boolean }): boolean {
...
function finalizeFloatingSelection(
	options?: { apply?: boolean; clearSelection?: boolean; sheet?: typeof currentSheet.value },
) {
	const move = pixelSelectionMove.value;
	...
	if (apply && sheet && !sameSpot) {
		recordPaintUndoSnapshot(sheet);
		sampleCtx.clearRect(move.sourceRect.x, move.sourceRect.y, move.sourceRect.w, move.sourceRect.h);
		sampleCtx.putImageData(move.imageData, move.currentRect.x, move.currentRect.y);
		commitSampleCanvasEdit("Image edits pending save");
	}
...
function beginSpriteMove(annotation: Annotation, point: { x: number; y: number }) {
	...
	const maxX = Math.max(...items.map((item) => item.sourceRect.x + item.sourceRect.w));
	const maxY = Math.max(...items.map((item) => item.sourceRect.y + item.sourceRect.h));
	spriteMove.value = {
...
function updateSpriteMove(point: { x: number; y: number }) {
	...
	const dx = Math.max(
		-move.groupBounds.x,
		Math.min(rawDx, sampleCanvas.width - (move.groupBounds.x + move.groupBounds.w)),
	);
	const dy = Math.max(
		-move.groupBounds.y,
		Math.min(rawDy, sampleCanvas.height - (move.groupBounds.y + move.groupBounds.h)),
	);
	spriteMove.value = {
		...move,
...
function finalizeSpriteMove(options?: { apply?: boolean; sheet?: typeof currentSheet.value }) {
	const move = spriteMove.value;
	const sheet = options?.sheet ?? currentSheet.value;
	const apply = options?.apply ?? false;
	...
	for (const item of move.items) {
		const annotation = sheet?.annotations.find((entry) => entry.id === item.annotationId) ?? null;
		if (!annotation?.aabb) continue;
	...
	if (apply && sheet && !sameSpot) {
		for (const item of move.items) {
			sampleCtx.clearRect(item.sourceRect.x, item.sourceRect.y, item.sourceRect.w, item.sourceRect.h);
		}
		for (const item of move.items) {
			sampleCtx.putImageData(item.imageData, item.currentRect.x, item.currentRect.y);
		}
...
function getSelectedSpriteAnnotations(): Annotation[] {
	const selectionIds = atlasMoveSelectionIds.value.length > 0
		? atlasMoveSelectionIds.value
		: selectedId.value
			? [selectedId.value]
			: [];
...
function copySpriteSelectionToClipboard(): boolean {
	if (activeAtlasTool.value !== "sprite-move" || !isPaintableCurrentSheet.value) return false;
	...
	const bounds = {
		...
		w: Math.max(...selectedAnnotations.map((annotation) => annotation.aabb!.x + annotation.aabb!.w))
			- Math.min(...selectedAnnotations.map((annotation) => annotation.aabb!.x)),
		h: Math.max(...selectedAnnotations.map((annotation) => annotation.aabb!.y + annotation.aabb!.h))
...
function cutSpriteSelectionToClipboard(): boolean {
	...
	const selectedIds = new Set(getSelectedSpriteAnnotations().map((annotation) => annotation.id));
	if (selectedIds.size === 0) return false;

	recordPaintUndoSnapshot(sheet);
	for (const annotation of sheet.annotations) {
		if (!annotation.aabb || !selectedIds.has(annotation.id)) continue;
		sampleCtx.clearRect(annotation.aabb.x, annotation.aabb.y, annotation.aabb.w, annotation.aabb.h);
	}
	sheet.annotations = sheet.annotations.filter((annotation) => !selectedIds.has(annotation.id));
	setAtlasSelection([]);
	spriteMove.value = null;
...
function pasteSpriteClipboard(): boolean {
	const sheet = currentSheet.value;
	if (!sheet || !spriteClipboard || activeAtlasTool.value !== "sprite-move" || !isPaintableCurrentSheet.value) {
		return false;
	}

	if (spriteClipboard.bounds.w > sampleCanvas.width || spriteClipboard.bounds.h > sampleCanvas.height) {
	...
	for (const item of spriteClipboard.items) {
		...
		sampleCtx.putImageData(item.imageData, originX + item.relativeX, originY + item.relativeY);
		sheet.annotations.push(nextAnnotation);
		pastedIds.push(nextAnnotation.id);
	}

	setAtlasSelection(pastedIds);
	commitSampleCanvasEdit(`Pasted ${pastedIds.length} sprite${pastedIds.length === 1 ? "" : "s"}`);
	markDirty(true);
	return true;
}
...
function commitAtlasSelectionDrag() {
	...
	const ids = annotations.value
		.filter((annotation) => annotation.aabb && rectsIntersect(annotation.aabb, rect))
		.map((annotation) => annotation.id);
	setAtlasSelection(ids);
	statusText.value = ids.length > 0 ? `Selected ${ids.length} sprite${ids.length === 1 ? "" : "s"}` : "No sprites selected";
}
...
function nudgePixelSelection(dx: number, dy: number) {
	const current = pixelSelectionMove.value?.currentRect ?? paintPixelSelection.value;
	if (!current || !isPaintableCurrentSheet.value) return false;
	if (!pixelSelectionMove.value) {
		const imageData = sampleCtx.getImageData(current.x, current.y, current.w, current.h);
		paintFloatingSelectionCanvas(imageData);
		pixelSelectionMove.value = {
	...
	const move = pixelSelectionMove.value!;
	const target = clampSelectionTarget(move.currentRect, move.currentRect.x + dx, move.currentRect.y + dy);
	if (target.x === move.currentRect.x && target.y === move.currentRect.y) return false;
	pixelSelectionMove.value = {
		...move,
		currentRect: target,
...
async function handleFitView() {
	if (!imageWidth.value || !imageHeight.value || !scroller.value || !stage.value) return;
...
function handleLayerPointerDown(event: PointerEvent) {
	...
	if (activeAtlasTool.value === "sprite-move") {
		...
		if (!isPaintableCurrentSheet.value || !currentSheet.value) {
			statusText.value = "Sprite move currently supports PNG, JPG, and WEBP sheets";
			return;
		}
		...
		if (hit?.aabb) {
			if (event.shiftKey) {
				if (atlasMoveSelectionIds.value.includes(hit.id)) {
					setAtlasSelection(atlasMoveSelectionIds.value.filter((id) => id !== hit.id));
				} else {
	...
	if (activePaintTool.value) {
		...
		if (!isPaintableCurrentSheet.value || !currentSheet.value) {
			statusText.value = "Paint tools currently support PNG, JPG, and WEBP sheets";
			return;
		}
...
function handleLayerPointerMove(event: PointerEvent) {
	...
	if (paintStroke.value && activePaintTool.value) {
		...
		if (!point) return;
		const erase = activePaintTool.value === "erase";
		drawPaintLine(paintStroke.value.lastX, paintStroke.value.lastY, point.x, point.y, erase);
		paintStroke.value = { lastX: point.x, lastY: point.y };
		renderDisplayCanvas();
		return;
	}
...
const marqueePreviewStyle = computed(() => selectionStyle(marqueePreviewRect.value));
const marqueeSelectionStyle = computed(() =>
	pixelSelectionDrag.value ? {} : selectionStyle(pixelSelectionMove.value?.currentRect ?? paintPixelSelection.value)
);
...
const atlasSelectionPreviewStyle = computed(() => selectionStyle(atlasSelectionPreviewRect.value));

function isAnnotationCanvasSelected(annotation: Annotation) {
	return annotation.id === selectedId.value || atlasMoveSelectionIds.value.includes(annotation.id);
}

function boxStyle(annotation: Annotation, annIndex: number) {
	const isSelected = isAnnotationCanvasSelected(annotation);
	const moved = spriteMove.value?.items.find((item) => item.annotationId === annotation.id);
					...
					@pointercancel="handleLayerPointerCancel">
						<template v-for="(annotation, annIndex) in annotations" :key="annotation.id">
							...
							<button v-if="getPrimaryShapeKind(annotation) === 'rect' && annotation.aabb"
								...
								:style="[boxStyle(annotation, annIndex), (isPaintMode || activeAtlasTool === 'sprite-move') ? { pointerEvents: 'none' } : null]"
							...
							<button v-else-if="getPrimaryShapeKind(annotation) === 'point' && annotation.point"
								...
								:class="{ selected: isAnnotationCanvasSelected(annotation) }"
								:style="[pointStyle(annotation, annIndex), (isPaintMode || activeAtlasTool === 'sprite-move') ? { pointerEvents: 'none' } : null]"
								@pointerdown="handleShapePointerDown($event, annotation)"
								@pointermove="handleBoxPointerMove" @pointerup="handleBoxPointerUp"
								@pointercancel="handleBoxPointerUp"
								@contextmenu.stop="onBoxContextMenu($event, annotation)">
						...
						></div>
						<div
							v-if="activeAtlasTool === 'sprite-move' && atlasMoveSelectionDrag && atlasSelectionPreviewRect"
							class="pixel-selection-box preview"
