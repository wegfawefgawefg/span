...
function openCreateDialog() {
	const base = currentSheet.value ? filename(currentSheet.value.path).replace(/\.[^.]+$/, "") : "sheet";
...
<template>
	<div class="h-full flex flex-col gap-2 overflow-hidden bg-surface-1">
		<div class="flex flex-col gap-1.5 p-2 pb-0">
			<div v-if="platform === 'desktop' && effectiveRoot" class="grid grid-cols-2 gap-2">
				<button type="button" class="sheet-action-button" @click="openCreateDialog()">New PNG</button>
				<button
					type="button"
		...
		<div v-if="showCreateDialog" class="sheet-modal-backdrop">
			<form class="sheet-modal-card" @submit.prevent="createSheet">
				...
				<div class="sheet-modal-copy">
					<div>Folder: <span class="font-mono">{{ targetFolderLabel }}</span></div>
					<div>Creates a transparent PNG in the current sheet folder.</div>
				</div>
				<label class="sheet-modal-field">
					<span>Name</span>
					<input v-model="newSheetName" type="text" placeholder="sheet.png" autofocus />
				</label>
				...
				<div class="sheet-modal-actions">
					<button type="button" class="sheet-action-button ghost" @click="closeCreateDialog()">Cancel</button>
					<button type="submit" class="sheet-action-button primary">Create</button>
				</div>
			</form>
		</div>
		<div v-if="showDeleteDialog && pendingDeleteSheet" class="sheet-modal-backdrop">
			<div class="sheet-modal-card">
				...
				<div class="sheet-modal-copy">
					<div>Delete <span class="font-mono">{{ filename(pendingDeleteSheet.path) }}</span> from disk?</div>
					...
					<div v-else>
						This only removes the file and sheet entry.
					</div>
				</div>
				<div class="sheet-modal-actions">
					<button type="button" class="sheet-action-button ghost" @click="closeDeleteDialog()">Cancel</button>
					<button type="button" class="sheet-action-button danger" @click="handleDeleteSheet(pendingDeleteSheet)">Delete</button>
				</div>
			</div>
		</div>
...
.sheet-action-button.primary {
	...
	background: color-mix(in srgb, var(--color-copper-glow) 55%, var(--color-surface-1));
	color: var(--color-copper-bright);
}
